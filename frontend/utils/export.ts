...
export function buildExportText(plan: FestivalPlan, polygon: PolygonPoint[]): string {
  ...
  return [
    ...
    `H₂: ${plan.energy.hydrogen_generators} · Solar: ${plan.energy.solar_panels} panels (${fmt(plan.energy.solar_daily_kWh)} kWh/day) · Batteries: ${plan.energy.battery_units} · Grid: ${plan.energy.grid_kw} kW`,
    ``, `## Resources`,
    ...
    `H₂: €${fmt(plan.cost.hydrogen_generators_eur)} · Solar: €${fmt(plan.cost.solar_panels_eur)} · Batteries: €${fmt(plan.cost.batteries_eur)} · Grid: €${fmt(plan.cost.grid_connection_eur)}`,
    polygonStr,
  ].join("\n");
}
